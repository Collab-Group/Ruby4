...
namespace MOSA1.Apps
{
    public class Terminal : Window
    {
        ...
        private unsafe void ProcessCommand()
        {
            ...
            {
                WriteLine("Moved to 'Machine Infomation and Data'");
                System.Add(new SystemInfo() { X = 433, Y = 350, Width = 300, Height = 170 });
            }
            else if (CMD == "NOTEPAD")
            {
            ...
            {
                System.MessageBox.Show(this, "Snake - Made by Nifanfa");
                System.Add(new Snake() { X = 100, Y = 520, Width = 10, Height = 10 });

            }
            else if (CMD == "HELP")
            {
                System.Panicked = true;
                WriteLine("About - About  Ruby4");
                WriteLine("Clear - Clears  terminal  history");
                WriteLine("FPS - Shows  refresh rate  of  display");
                WriteLine("Shutdown -  Powers  off  computer");
                WriteLine("Reboot - Reboots  the  computer");
                WriteLine("VBE - Graphics  status");
                WriteLine("FPS - Frames  Per  Second  counter,  a  continuable  command");
                WriteLine("Diagnostic - Frames  Per  Second  counter,  a  continuable  command");
                WriteLine("Beep - PC Speaker beep");
            }
            else if (CMD == "CLEAR")
            {
                CursorX = 0;
